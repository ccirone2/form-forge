...
| `placeholder` | string | Input placeholder text. Not applicable to `date`, `time`, `datetime`, `heading`, `hidden`, `address`, `file`, `signature`, `repeater`. |
...
**Template:** Value may contain `\n` for paragraph breaks. Use the stencils helper (recommended):
...
**Template:** Value is a newline-separated string. Empty list produces `""`. Use the stencils helper (recommended):
...
**Template:** Value is a JSON string. Use the stencils helper (recommended):
...
**Template:** Value is a base64 data URI string (e.g., `"data:image/png;base64,iVBOR..."`) or empty string if no file was uploaded. Use the stencils helper (recommended):

```python
stencils.image(doc, data.get('receipt_photo', ''), width_inches=3.0)
```
...
**Template:** Value is a base64 PNG data URI string or empty string. Use the stencils helper (recommended):
...
**Sub-field optional properties:**

| Property | Supported sub-field types | Notes |
|----------|--------------------------|-------|
| `placeholder` | `text`, `email`, `tel`, `number`, `currency`, `url` | Not rendered for `select`, `time`, `toggle`. |
| `hint` | *(all)* | Accepted by the schema spec but **not rendered** in the browser for sub-fields. |
| `required` | *(all)* | Accepted by the schema spec but not enforced in the browser for sub-fields. |
| `options` | `select` | Required for `select` sub-fields; forbidden on all others. |
| `min`, `max`, `step` | `number` | Forbidden on all other sub-field types. |
| `currency_symbol` | `currency` | Defaults to `"$"`. Forbidden on all other sub-field types. |
| `maxLength` | `text` | Accepted by the schema spec but not enforced in the browser for sub-fields. |
...
**Template:** Value is a JSON array string. Use the stencils helper for table output (recommended):
...
- `text`, `email`, `tel`, `date`, `time`, `url`, `datetime`, `textarea`, `longtext`, `number`, `currency`, `toggle` — attaches `input` and `change` listeners on the element
...
Does **not** reliably work as a target for: `checkbox`, `radio`, `list`, `address`, `repeater`, `heading`, `info` — these types do not render a single element with `id = field.id`, so the visibility logic cannot find their field group.
