...
#### Legacy `COLOR_*` constants (backward compatible)

The five original constants still exist and always reflect the active palette:

| Constant | Palette role |
|---|---|
| `stencils.COLOR_MEDIUM_BLUE` | `title` |
| `stencils.COLOR_SOFT_BLUE` | `subtitle` |
| `stencils.COLOR_MUTED` | `muted` |
| `stencils.COLOR_LIGHT_MUTED` | `footer` |
| `stencils.COLOR_DARK_NAVY` | `accent` |

These are updated automatically when you call `set_palette()`. New code should use the palette system directly.
...
fr.font.size = Pt(8)
fr.font.color.rgb = stencils.COLOR_LIGHT_MUTED  # or stencils.PALETTE_CLASSIC.footer
fr.italic = True
```
